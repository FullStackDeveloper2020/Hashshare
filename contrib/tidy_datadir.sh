...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete HashShareCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no HashShareCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old HashShareCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected HashShareCoin 0.7 datadir."
    ;;
  3)
    echo "Detected HashShareCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected HashShareCoin 0.8 datadir."
    ;;
esac
