...
#if defined(NDEBUG)
# error "HashShareCoin Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "HashShareCoin Signed Message:\n";
                              ...
                              const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    BOOST_FOREACH(const CTxIn &txin, tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in HashShareCoin
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	if (nHeight == 0) {
		...
		nSubsidy = 402250 * COIN;
	}
	else if (nHeight <= 60480 && nHeight > Params().GetConsensus().nLastPoWBlock) {
		nSubsidy = 192.5 * COIN;
	}
	else if (nHeight <= 280000 && nHeight > 60480) {
		nSubsidy = 96.25 * COIN;
	}
	else if (nHeight <= 720000 && nHeight > 280000) {
		nSubsidy = 48.13 * COIN;
	}
	else if (nHeight <= 1650000 && nHeight > 720000) {
		nSubsidy = 24.06 * COIN;
	}
	else if (nHeight <= 3450000 && nHeight > 1650000) {
		nSubsidy = 12.03 * COIN;
	}
	else if (nHeight <= 5850000 && nHeight > 3450000) {
		nSubsidy = 6.02 * COIN;
	}
	else if (nHeight <= 10512000 && nHeight > 5850000) {
		nSubsidy = 1.53 * COIN;
	}
	else if (nHeight <= 10541440 && nHeight > 10512000) {
		nSubsidy = 1 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
	...
	else if (nHeight <= 10541440 && nHeight > Params().GetConsensus().nLastPoWBlock) {
...
static int64_t nTimeHashShareCoinSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // HSS : CHECK TRANSACTIONS FOR INSTANTSEND
        ...
        for (const auto& tx : block.vtx) {
            ...
            for (const auto& txin : tx->vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                    ...
                    return state.DoS(10, error("ConnectBlock(HSS): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), hashLocked.ToString()),
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
                return state.DoS(10, error("ConnectBlock(HSS): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(HSS): spork is off, skipping transaction locking checks\n");
    }
    ...
    // HSS : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(HSS): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (FullDIP0003Mode() || pindex->nHeight == Params().GetConsensus().nGenerationHeight) {
       if (!IsBlockPayeeValid(*block.vtx[isProofOfStake], pindex->nHeight, blockReward - nFees)) {
           mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
           return state.DoS(0, error("ConnectBlock(HSS): couldn't find masternode or superblock payments"), REJECT_INVALID, "bad-cb-payee");
       }
    }
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(HSS): ProcessSpecialTxsInBlock for block failed with %s", FormatStateMessage(state));
    }
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeHashShareCoinSpecific += nTime5 - nTime4;
    LogPrint("bench", "    - HashShareCoin specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeHashShareCoinSpecific * 0.000001);
