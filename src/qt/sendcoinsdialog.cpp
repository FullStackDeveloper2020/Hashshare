    ...
    platformStyle(_platformStyle)
{
    ...
    if(fLiteMode) {
        ...
        ui->checkUseInstaHSS->setVisible(false);
        ...
        ui->checkUseInstaHSS->setChecked(fUseInstantSend);
    ...
    connect(ui->checkUseInstaHSS, SIGNAL(stateChanged ( int )), this, SLOT(updateInstaHSS()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if(model->IsOldInstantSendEnabled() && ui->checkUseInstaHSS->isChecked()) {
        strFunds += " ";
        strFunds += tr("and InstaHSS");
    }

    for (SendCoinsRecipient& rcp : recipients) {
        rcp.inputType = ui->checkUsePrivateSend->isChecked() ? ONLY_DENOMINATED : ALL_COINS;
        rcp.fUseInstantSend = model->IsOldInstantSendEnabled() && ui->checkUseInstaHSS->isChecked();
    }
...
void SendCoinsDialog::updateInstaHSS()
{
    QSettings settings;
    settings.setValue("bUseInstantX", ui->checkUseInstaHSS->isChecked());
    CoinControlDialog::coinControl->fUseInstantSend = model->IsOldInstantSendEnabled() && ui->checkUseInstaHSS->isChecked();
    coinControlUpdateLabels();
}
