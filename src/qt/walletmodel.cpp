...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered hashsharecoin address / amount:
    ...
    if(recipients[0].fUseInstantSend && IsOldInstantSendEnabled() && total > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
        Q_EMIT message(tr("Send Coins"), tr("InstaHSS doesn't support sending values that high yet. Transactions are currently limited to %1 HSS.").arg(sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)),
    ...
    if(recipients[0].fUseInstantSend && IsOldInstantSendEnabled()) {
        ...
        if(nVinSize > CTxLockRequest::WARN_MANY_INPUTS) {
            Q_EMIT message(tr("Send Coins"), tr("Used way too many inputs (>%1) for this InstaHSS transaction, fees could be huge.").arg(CTxLockRequest::WARN_MANY_INPUTS),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal hss:URI (hss:XyZ...?message=example)
            {
...
bool WalletModel::transactionCanBeAbandoned(uint256 hash) const
{
    ...
    if (!wtx || wtx->isAbandoned() || wtx->GetDepthInMainChain() > 0 || wtx->IsLockedByInstaHSS() || wtx->InMempool())
        return false;
    return true;
}
