...
QString TransactionDesc::FormatTxStatus(const CWalletTx& wtx)
{
    ...
    else
    {
        ...
        if (wtx.IsLockedByLLMQInstaHSS()) {
            strTxStatus += " (" + tr("verified via LLMQ based InstaHSS") + ")";
            return strTxStatus;
        }
        ...
        if(instantsend.IsLockedInstantSendTransaction(wtx.GetHash())) {
            strTxStatus += tr("verified via InstaHSS");
        } else if(!instantsend.IsTxLockCandidateTimedOut(wtx.GetHash())) {
            strTxStatus += tr("InstaHSS verification in progress - %1 of %2 signatures").arg(nSignatures).arg(nSignaturesMax);
        } else {
            strTxStatus += tr("InstaHSS verification failed");
        }
        strTxStatus += ")";
...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    // Message from normal hss:URI (hss:XyZ...?message=example)
