...
void PrepareShutdown()
{
    ...
    if (!fLiteMode && !fRPCInWarmup) {
        ...
        {
            CFlatDB<CInstantSend> flatdb5("instantsend.dat", "magicInstaHSSCache");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via InstaHSS) in <address>"));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashinstantsenddoublespend=<address>", _("Enable publish transaction hashes of attempted InstaHSS double spend in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via InstaHSS) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawinstantsenddoublespend=<address>", _("Enable publish raw transactions of attempted InstaHSS double spend in <address>"));
#endif
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "hashsharecoin (or specifically: chainlocks, gobject, instantsend, keepass, llmq, llmq-dkg, llmq-sigs, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode", strprintf(_("Disable all HashShareCoin specific functionality (Masternodes, PrivateSend, InstaHSS, Governance) (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-sporkaddr=<hashsharecoinaddress>", strprintf(_("Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.")));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N HSS anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
    ...
    strUsage += HelpMessageGroup(_("InstaHSS options:"));
    strUsage += HelpMessageOpt("-enableinstantsend", strprintf(_("Enable InstaHSS, show confirmations for locked transactions (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-instantsendnotify=<cmd>", _("Execute command when a wallet InstaHSS transaction is successfully locked (%s in cmd is replaced by TxID)"));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/HashShareCoinOfficial/HSS>";
    const std::string URL_WEBSITE = "<https://hashsharecoin.io>";
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("HashShareCoin Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single HashShareCoin Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    // lite mode disables all HashShareCoin-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);
    LogPrintf("fLiteMode %d\n", fLiteMode);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all HashShareCoin-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/HashShareCoinOfficial/HSS/pull/1817 and https://github.com/HashShareCoinOfficial/HSS/pull/1743
    ...
    if (!fIgnoreCacheFiles) {
        ...
        if(fEnableInstantSend)
        {
            ...
            uiInterface.InitMessage(_("Loading InstaHSS data cache..."));
            CFlatDB<CInstantSend> flatdb5(strDBName, "magicInstaHSSCache");
    ...
    // ********************************************************* Step 10d: schedule HashShareCoin-specific tasks
