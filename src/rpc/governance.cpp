...
void gobject_prepare_help(CWallet* const pwallet)
{
    throw std::runtime_error(
                ...
                "5. use-IS        (boolean, optional, default=false) InstaHSS lock the collateral, only requiring one chain confirmation\n"
...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 HSS irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on hashsharecoincentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in hashsharecoin.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("hashsharecoin.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in hashsharecoin.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "hashsharecoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "hashsharecoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "hashsharecoin",               "gobject",                &gobject,                true,  {} },
    { "hashsharecoin",               "voteraw",                &voteraw,                true,  {} },
