...
const char * const BITCOIN_CONF_FILENAME = "hashsharecoin.conf";
const char * const BITCOIN_PID_FILENAME = "hashsharecoin.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                if(ptrCategory->count(std::string("hashsharecoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HashShareCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HashShareCoinCore
    // Mac: ~/Library/Application Support/HashShareCoinCore
    // Unix: ~/.hashsharecoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HashShare";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HashShare";
#else
    // Unix
    return pathRet / ".hashshare";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty hashsharecoin.conf if it does not excist
