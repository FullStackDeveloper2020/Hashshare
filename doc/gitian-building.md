...
*Setup instructions for a Gitian build of HashShareCoin Core using a Debian VM or physical system.*
...
- [Building HashShareCoin Core](#building-hashsharecoin-core)
...
Clone the git repositories for HashShareCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/hashsharecoinpay/hashsharecoin
...
in the HashShareCoin Core repository under 'Fetch and create inputs' to install sources which require
...
To build HashShareCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the HashShareCoin Core repository.
...
```bash
    ...
    From https://github.com/hashsharecoinpay/hashsharecoin
...
and the hashsharecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../hashsharecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/hashsharecoin
SIGPATH=/some/root/path/hashsharecoin-detached-sigs

./bin/gbuild --url hashsharecoin=${BTCPATH},signature=${SIGPATH} ../hashsharecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hashsharecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hashsharecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/hashsharecoin-osx-build.assert
```
...
[hashsharecoin/gitian.sigs](https://github.com/hashsharecoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
