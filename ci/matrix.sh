...
export BUILDER_IMAGE_NAME="hashsharecoin-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/hashsharecoin"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_HASHSHARECOIN_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
